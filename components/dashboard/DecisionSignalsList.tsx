...
export function DecisionSignalsList({ signals, onSelect }: { signals: DecisionSignal[]; onSelect: (signal: DecisionSignal) => void }) {
  ...
  return (
    <section className="rounded-2xl border border-[#dce7e1] bg-white shadow-[0_4px_16px_rgba(25,65,49,0.04)]">
      <div className="flex items-center justify-between px-6 pb-4 pt-6 sm:px-7"><div><h2 className="text-lg font-semibold tracking-[-0.025em] text-[#244b3d]">Decision signals</h2><p className="mt-1 text-sm text-[#71837a]">{hasSignals ? "The findings that most affect execution readiness." : "No material findings are supported by the available evidence."}</p></div><span className="hidden rounded-full bg-[#edf5f0] px-2.5 py-1 text-xs font-medium text-[#4d7867] sm:block">{signals.length} {signals.length === 1 ? "signal" : "signals"}</span></div>
      <div className="border-t border-[#edf2ef]">
        {hasSignals ? signals.map((signal) => (
        ...
        )) : <div className="flex items-start gap-3 px-6 py-7 sm:px-7"><span className="flex size-9 shrink-0 items-center justify-center rounded-lg bg-[#f2f6f3] text-[#5b7a6d]"><FileSearch className="size-4" aria-hidden="true" /></span><p className="max-w-xl text-sm leading-6 text-[#61766c]">The submitted documents did not provide enough distinct evidence for a material decision signal. Review the assessment summary and business exposure before proceeding.</p></div>}
      </div>
      {hasSignals && <div className="px-6 py-4 sm:px-7"><p className="flex items-center gap-1.5 text-xs font-medium text-[#5d7b6e]"><ArrowUpRight className="size-3.5" aria-hidden="true" /> Select a signal to review its evidence and recommendation.</p></div>}
    </section>
  );
}
